...
class app(tk.Frame):
    ...
    def __init__(self, master):
        ...
        self.upload_data = tk.Entry(self)
        self.label_1 = tk.Label(self, text="Enter the value to be uploaded:")
        self.label_1.grid(row = 0, column = 0)
        self.upload_data.grid(row = 0, column = 1)


        self.button1 = tk.Button(self, text="Upload Data", command=self.thingspeak_post)
        self.button1.grid(row = 1, columnspan = 2)

        self.get_data = tk.Entry(self)
        self.label_2 = tk.Label(self, text="Enter the number of values to be retrieved:")
        self.label_2.grid(row=3, column=0)
        self.get_data.grid(row=3, column=1)

        self.button2 = tk.Button(self, text = "Get Data" , command = self.thingspeak_get)
